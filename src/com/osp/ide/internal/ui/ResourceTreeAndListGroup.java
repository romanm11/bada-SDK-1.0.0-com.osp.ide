...
public class ResourceTreeAndListGroup extends EventManager implements
    ...
    public List getAllCheckedItems() {

        List result = new ArrayList();

        if (root instanceof ArrayList) {
            //Object[] children = treeContentProvider.getChildren(((ArrayList) root).get(0));
        	Object[] children = ((ArrayList) root).toArray();
            for (int i = 0; i < children.length; ++i) {
                findAllWhiteCheckedItems(children[i], result);
            }
    ...
    public void selectionChanged(SelectionChangedEvent event) {
        ...
        if (!selectedElement.equals(currentTreeSelection)) {
